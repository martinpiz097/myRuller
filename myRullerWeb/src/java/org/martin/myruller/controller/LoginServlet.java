...
public class LoginServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ElectroList<User> elements = dao.getElements();
    
        User loged = elements.findFirst(
                u -> u.getNick().equals(user) && u.getPassword().equals(passw));
        
        if (loged == null)
            response.sendRedirect("index.jsp?e="+Error.LOGIN);
        else{
            ClientDAO cliDao = new ClientDAO();
            request.getSession().setAttribute("client", cliDao.getClientByIdUser(loged.getId()));
